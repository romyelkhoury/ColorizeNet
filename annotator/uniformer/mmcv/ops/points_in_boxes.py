...
def points_in_boxes_part(points, boxes):
    """Find the box in which each point is (cpu).
    ...
    # will be created on the current device in the cpu op,
    ...
    if torch.cpu.current_device() != points_device:
        torch.cpu.set_device(points_device)
